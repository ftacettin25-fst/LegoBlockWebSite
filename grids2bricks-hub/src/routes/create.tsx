  ...
  head: () => ({
    meta: [
      { title: "Create your BrickHeadz — Grids2Bricks" },
      {
        name: "description",
        content:
          "Upload a photo, customize colors and style, preview your BrickHeadz in 3D, and add it to your cart.",
      },
      { property: "og:title", content: "Create your BrickHeadz" },
      {
        property: "og:description",
        content: "Design your custom BrickHeadz figurine in real-time 3D.",
      },
    ],
  }),
...
function LDrawModel({ url }: { url: string }) {
  const [model, setModel] = useState<THREE.Group | null>(null);

  useEffect(() => {
    const loader = new LDrawLoader();
    loader.setPartsLibraryPath("/static/ldraw/");

    loader
      .preloadMaterials("/static/ldraw/LDConfig.ldr")
      .then(() => {
      ...
      .catch((e) => {
        console.warn("Could not preload LDConfig, loading without materials", e);
        loader.load(url, (group) => {
          const box = new THREE.Box3().setFromObject(group);
          const center = box.getCenter(new THREE.Vector3());
          group.position.set(-center.x, -center.y, -center.z);
...
function CreatePage() {
  ...
  const [personData, setPersonData] = useState<Record<string, unknown> | null>(null);
  ...
  const handleBuild = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setLdrUrl(data.ldr_url);
        setDownloadUrl(data.download_url);
        setPersonData(data.person_data);

        // Update color optionally if the backend sends one (disabled since backend handles fully)
        // We'll just display it.
        setBuilt(true);
        toast.success("Your BrickHead is built! Preview it below.");
        setTimeout(() => {
          document
            .getElementById("build-output")
  ...
  const handleBuildDone = () => {
    // Left empty. Built flag is toggled natively in handleBuild now.
  };
  ...
  const addToCart = async () => {
    ...
    try {
      ...
      // We will later save the job data to Firebase here using the regular firebase sdk.
      // For now, we simply add to local cart context.
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold tracking-tight sm:text-4xl">
            Create your BrickHeadz
          </h1>
          <p className="mt-2 text-muted-foreground">
            Upload, customize, preview in 3D, and add to cart.
          </p>
        </div>
            ...
            <div className="mt-4 space-y-4">
              <div>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  Select multiple extras to combine them. They appear in the 3D preview.
                </p>
              </div>
              <div>
                <Label className="text-xs uppercase tracking-wider text-muted-foreground">
          ...
          <div id="build-output" className="mt-10 space-y-6">
            <div>
              ...
              <p className="mt-1 text-muted-foreground">
                Download instructions, place your order, and explore the 3D preview.
              </p>
            </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <Button variant="outline" asChild>
                    <a href={downloadUrl || "#"} download>
                      <Download className="h-4 w-4 mr-2" /> Download LDR
                    </a>
                  </Button>
                  {/* Note: The old PDF instruction engine would go here. For now we provide the raw LDR. */}
                  <Button
                    onClick={addToCart}
                    disabled={submitting}
            ...
            <Card className="overflow-hidden p-0">
              ...
              {extras.length > 0 && (
                <div className="border-t bg-card p-4 text-center">
                  ...
                  <div className="mt-1 font-bold tracking-wide capitalize">
                    {extras.join(" + ")}
                  </div>
                </div>
              )}
