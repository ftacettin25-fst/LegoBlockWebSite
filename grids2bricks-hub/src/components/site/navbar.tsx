  ...
  return (
    ...
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link
          to="/"
          className="flex items-center gap-1 text-xl font-extrabold tracking-tight text-nav-foreground"
        >
          <span>Grids</span>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {links.map((l) => (
            <Link
              ...
              activeProps={{
                className:
                  "rounded-md px-3 py-2 text-sm font-semibold text-nav-foreground bg-white/10",
              }}
          ...
          {user ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  className="inline-flex h-9 w-9 items-center justify-center rounded-md text-nav-foreground/80 transition-smooth hover:bg-white/10 hover:text-nav-foreground"
                  aria-label="Account"
                >
            ...
            <Link to="/auth" className="hidden sm:inline-flex">
              <Button
                size="sm"
                variant="secondary"
                className="bg-white/10 text-nav-foreground hover:bg-white/20 border-0"
              >
                Sign in
              </Button>
            </Link>
        ...
        <div className="md:hidden border-t border-white/10 px-4 pb-4 pt-2">
          {links.map((l) => (
            <Link
              ...
              activeProps={{
                className:
                  "block rounded-md px-3 py-2.5 text-sm font-semibold text-nav-foreground bg-white/10",
              }}
              activeOptions={{ exact: true }}
            >
              {l.label}
